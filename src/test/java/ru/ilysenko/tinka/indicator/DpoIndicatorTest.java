...
import ru.ilysenko.tinka.tools.indicator.DpoIndicator;
import ru.ilysenko.tinka.tools.indicator.Indicator;

import static org.junit.jupiter.api.Assertions.assertEquals;
import static org.mockito.Mockito.spy;

class DpoIndicatorTest extends IndicatorTest {

    @Test
    void period1() {
        Indicator indicator = DpoIndicator.create().periodsCount(1).init();
    ...
    @Test
    void period2() {
        Indicator indicator = DpoIndicator.create().periodsCount(2).init();
        double result = indicator.calculate(getCandles());
        assertEquals("-0.3300", format(result));
    }

    @Test
    void period3() {
        Indicator indicator = DpoIndicator.create().periodsCount(3).init();
        double result = indicator.calculate(getCandles());
        assertEquals("-0.4733", format(result));
    }

    @Test
    void testOverbought() {
        testOverboughtState(0.02);
    }

    @Test
    void testOversold() {
        testOversoldState(-0.04);
    }

    protected Indicator makeIndicator() {
        return spy(DpoIndicator.class);
    }
}